...
func drone_harvest() -> bool:
        # v1.0.4 Utility Drone harvest: bypasses the player's Field Scanner
        # gate (the drone carries its own scanner array). Same charge rules.
        if respawn_timer > 0.0 or charges <= 0:
                return false
        var qty := int(def.get("qty", 1))
        charges -= 1
        harvested.emit(self, def["item"], qty)
        Game.add_item(def["item"], qty)
        Game.toast.emit("Drone: +%d %s" % [qty, Data.item_name(def["item"])], Color(0.75, 0.9, 1.0))
        if charges <= 0:
                respawn_timer = float(def.get("respawn", 30.0))
                _set_visual_active(false)
        return true
