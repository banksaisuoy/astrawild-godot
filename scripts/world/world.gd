...
func _build_camp() -> void:
	...
	var maren := Node3D.new()
	maren.set_script(npc_script)
	maren.position = Vector3(cx + 5.0, tile_height(cx + 5.0, cz + 4.0), cz + 4.0)
	maren.set_meta("npc_name", "Warden Maren")
	maren.set_meta("npc_color", Color(0.4, 0.85, 0.65))
	maren.set_meta("npc_greeting", "The fields are calm — for now. The dawnwood and fieldstone will make you ready.")
	maren.set_meta("npc_role", "quest")
	npcs_root.add_child(maren)

	var tam := Node3D.new()
	tam.set_script(npc_script)
	tam.position = Vector3(cx - 5.0, tile_height(cx - 5.0, cz + 4.0), cz - 0.0 + 4.0)
	tam.set_meta("npc_name", "Trader Tam")
	tam.set_meta("npc_color", Color(0.9, 0.75, 0.35))
	tam.set_meta("npc_greeting", "Shards, friend. Shards for everything.")
	tam.set_meta("npc_role", "vendor")
	npcs_root.add_child(tam)

	var wren := Node3D.new()
	wren.set_script(npc_script)
	wren.position = Vector3(cx - 6.0, tile_height(cx - 6.0, cz - 6.0), cz - 6.0)
	wren.set_meta("npc_name", "Herbalist Wren")
	wren.set_meta("npc_color", Color(0.7, 0.85, 0.4))
	wren.set_meta("npc_greeting", "Feed what you befriend. Every Echo has a favorite.")
	wren.set_meta("npc_role", "herbalist")
	npcs_root.add_child(wren)
...
func _build_locations() -> void:
	# Hollow Underlight gate (Hollow Approach, 1.2 km west of zone center)
	...
	# boss: Underlight Warden guards the gate
	_spawn_boss("Creature_UnderlightWarden", Vector3(gate_x + 14.0, tile_height(gate_x + 14.0, gate_z), gate_z))

	# Driftwood Landing (Tidebreaker Isles hamlet)
	...
	_add_campfire(dl + Vector3(2.0, 0.0, 1.0), false)
	var elder := Node3D.new()
	elder.set_script(load("res://scripts/world/npc.gd"))
	elder.position = dl + Vector3(-2.0, 0.0, 2.0)
	elder.set_meta("npc_name", "Elder Rowan")
	elder.set_meta("npc_color", Color(0.5, 0.75, 0.85))
	elder.set_meta("npc_greeting", "Fisherfolk came before the flood. The vault below still hums — mind the colossus.")
	elder.set_meta("npc_role", "quest")
	npcs_root.add_child(elder)

	# Sunken Vault (Tidebreaker Isles)
	...
	if vault_arch:
		var mat := StandardMaterial3D.new()
		mat.albedo_color = Color(0.3, 0.45, 0.55)
		mat.emission_enabled = true
		mat.emission = Color(0.2, 0.6, 0.9)
		mat.emission_energy_multiplier = 1.4
		vault_arch.material_override = mat
	_spawn_boss("Creature_VaultColossus", sv + Vector3(10.0, 0.0, 6.0))
